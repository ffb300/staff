 ...
 * @since  1.0.0
 */
class NameformModel extends FormModel
{
	private $item = null;
	...
	{
		// Get the form.
		$form = $this->loadForm('com_staff.name', 'nameform', array(
						'control'   => 'jform',
						'load_data' => $loadData
				)
		);
	...
	{
		$data = Factory::getApplication()->getUserState('com_staff.edit.name.data', array());
	...
	public function delete($id)
	{
		...
		if (empty($id))
		{
			$id = (int) $this->getState('name.id');
		}

		if ($id == 0 || $this->getItem($id) == null)
		{
				throw new \Exception(Text::_('COM_STAFF_ITEM_DOESNT_EXIST'), 404);
		}

		if ($user->authorise('core.delete', 'com_staff') !== true)
		{
				throw new \Exception(Text::_('JERROR_ALERTNOAUTHOR'), 403);
		}
