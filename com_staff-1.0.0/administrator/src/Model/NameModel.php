 ...
 * @since  1.0.0
 */
class NameModel extends AdminModel
{
	/**
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_staff.name';
	...
	public function duplicate(&$pks)
	{
		...
		foreach ($pks as $pk)
		{
			if(!$pk || $this->userIDItem($pk) || $this->isAdminOrSuperUser())
			{
				...
				{
					if (is_array($table->position))
					{
						$table->position = implode(',', $table->position);
					}
				}
				else
				{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__staff_name');
				$max             = $db->loadResult();
